...
class TestCompareBatchDAO(TestCase):
    ...
    def test_get_batch_by_id(self):
        ...
        with get_local_connection() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    """SELECT skill, rb_start_date FROM report_batch WHERE batch_id = 'TR-1145'"""
                )
    ...
    def test_get_batches_same_skill(self):
        test_batch_ids = dao.get_batches_with_same_skill(
            "PEGA", "2021-05-28", productionDB=False
        )
        with get_local_connection() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    """SELECT batch_id FROM report_batch WHERE skill = 'PEGA' AND '2021-05-28' > rb_start_date"""
                )
    ...
    def test_batch_total_avg(self):
        ...
        with get_local_connection() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    """SELECT SUM(average)/ COUNT(average) FROM batch_grades WHERE batch_id = 'TR-1140'"""
                )
    ...
    def test_batch_weekly_avg(self):
        ...
        with get_local_connection() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    """SELECT SUM(score)/COUNT(score) FROM report_on_assessment
